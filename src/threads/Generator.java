...
public class Generator extends Thread {
    ...
    @Override
    public void run() {
        Random rand = new Random();//Создаём объект Random для генерации случайных чисел
        for (int i = 0; i < task.getTaskCount(); i++) {
            // Генерация всех данных заранее
            double base = 1 + 9 * rand.nextDouble();//Генерация основания логарифма [1, 10)
            double left = 100 * rand.nextDouble();//Левая граница [0, 100)
            double right = left + 100 * rand.nextDouble();//Правая граница [left, left+100)
            double step = 0.01 + 0.99 * rand.nextDouble();//Шаг [0.01, 1)
