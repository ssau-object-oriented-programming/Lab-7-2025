...
public class ArrayTabulatedFunction implements TabulatedFunction, Serializable{
    ...
    public ArrayTabulatedFunction(FunctionPoint[] points) {
        if (points.length < 2) {
                throw new IllegalArgumentException("Количество точек (" + points.length + 
                    ") должно быть не менее 2");
            }
    ...
    public double getPointY(int index){
     ...
     public String toString() {
        ...
        for (int i = 0; i < pointsCount; i++) {
            sb.append(points[i]); // используем toString() FunctionPoint
            if (i < pointsCount - 1) {
                sb.append(", ");
            }
        }
        ...
        public boolean equals(Object o) {
            ...
            if (pointsCount != other.getPointsCount()) return false;
             ...
             // ДЕЛЕГИРУЕМ сравнение каждой точки методу equals класса FunctionPoint
            for (int i = 0; i < pointsCount; i++) {
                FunctionPoint thisPoint = this.getPoint(i);      // (x1, y1)
                FunctionPoint otherPoint = other.getPoint(i);    // (x2, y2)
                
                // Вот здесь полное делегирование!
